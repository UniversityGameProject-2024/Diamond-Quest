...
public class RandomSpawner : MonoBehaviour
{
    ...
    private IEnumerator SpawnBoss()
    {
        while (true)
        {
            ...
            {
                Vector3 bossPosition = new Vector3(0.8f, 0.2f, 8f);
                Vector3 bossWorldPosition = mainCamera.ViewportToWorldPoint(bossPosition);
    ...
    public void SpawnRandomDiamond()
    {
        ...
        Vector3 worldPosition = new Vector3(0.5f, 0.5f, 0.5f);
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            Collider[] colliders = Physics.OverlapSphere(worldPosition, MIN_DISTANCE_BETWEEN_DIAMONDS);
            if (colliders.Length == 0)
            {
                Debug.Log("✅ Diamond spawned without collision.");
                break;
            }
            else
            {
                Debug.Log("❌ Diamond collides with another collider.");
            }
        }
